...
sbdb = pd.read_csv("sbdb_query_results_NEA.csv", low_memory=False) # Take in SBDB website data
...
#        break
    ...
    if counter % 2000 == 0: # If we have reached a milestone such as 2000 searched make a progress save
        progressCheck += 1 # Add one to progress check
        print("Making Save #{}".format(progressCheck)) # Make note of saving
        filepath = Path('C:/Users/Kedree Proffitt/Downloads/NASA API JIG/progress{}.csv'.format(progressCheck)) # Set filepath to Downloads and
        # put the progresscheck number at the end, ie. progress1.csv, progress2.csv
        filepath.parent.mkdir(parents=True, exist_ok=True) # Not exactly sure what this does, think its needed for file saving
        completeData.to_csv(filepath, index=False) # Save the data as a csv, do not reindex the data
...
filepath = Path('C:/Users/Kedree Proffitt/Downloads/NASA API JIG/finishedNEA.csv') # The file will be saved to this filepath
